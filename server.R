...
library(shiny)
library(ggplot2)
library(dplyr)#làm sạch số liệu
library(tidyr)
library(DT)
library(knitr)
library(kableExtra)#create table
library(ggthemes)
library(plotly)
library(rsconnect)
library(shinythemes)#hô trợ css
...
server <- function(session, input, output) {
  ...
  #output$datahead <- renderTable({
   # stateFilter <- subset(data_e, data_e$year=input$checkYear)
  #})
  output$datahead <- renderText({
    data %>%
  ...
  output$piePlot <- renderPlot({
    ...
    data_g %>%
      ...
      labs(title = "Tỷ lệ sinh viên tốt nghiệp ở các trường Đại học")
    #lengend: vị trí chú giải, tiêu đề, scale_fill_manual là hàm thẩm mĩ, 
  })
  ...
  # ----------------
  # scatter plot section - phần biểu đồ phân tán
  # ----------------
